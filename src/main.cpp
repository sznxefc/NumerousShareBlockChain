...
unsigned int nStakeMinAge = 60 * 60 * 24;
...
int64 nChainStartTime = 1525338801;
...
const string strMessageMagic = "NumerousShareBlockChain Signed Message:\n";
...
// miner's coin base reward based on nBits —— POW共识机制下，每挖出一个矿给矿工奖励多少个币
...
bool CBlock::ConnectBlock(CTxDB& txdb, CBlockIndex* pindex, bool fJustCheck)
{
    ...
    bool fEnforceBIP30 = true; // Always active in NumerousShareBlockChain
    bool fStrictPayToScriptHash = true; // Always active in NumerousShareBlockChain
...
bool CheckDiskSpace(uint64 nAdditionalBytes)
{
    ...
    if (nFreeBytesAvailable < nMinDiskSpace + nAdditionalBytes)
    {
        ...
        printf("*** %s\n", strMessage.c_str());
        uiInterface.ThreadSafeMessageBox(strMessage, "NumerousShareBlockChain", CClientUIInterface::OK | CClientUIInterface::ICON_EXCLAMATION | CClientUIInterface::MODAL);
        StartShutdown();
        return false;
    }
    return true;
...
bool LoadBlockIndex(bool fAllowNew)
{
    ...
    if (mapBlockIndex.empty())
    {
        ...
        // 创世块信息 ↓↓↓↓↓↓↓↓↓↓↓↓↓↓↓↓↓↓
        const char* pszTimestamp = "NumerousShareBlockChain created by Mr.X 2018/10/15";
        CTransaction txNew;
       ...
       // printf("block.nNonce = %u \n", block.nNonce);
        ...
        assert(block.hashMerkleRoot == uint256("0x5cf15414823a8cea114aa01b91518a2f15c6856f1346ecc8a38cb1b4ef7d85b6"));//  #?2 创世块哈希值？改过创世块的信息，这里就要改——第一次编译运行出错后再改这里（看教程）
